...
public class ST_PuzzleDisplay : MonoBehaviourPunCallbacks 
{
	// this puzzle texture.
	...
	public IEnumerator CheckForComplete()
	{
		while(Complete == false)
		{
			...
			for(int j = Height - 1; j >= 0; j--)
			{
				for(int i = 0; i < Width; i++)
				{
					var a = TileDisplayArray[i, j].GetComponent<ST_PuzzleTile>();
					// check if this tile has the correct grid display location.
					if(a.CorrectLocation == false)  
					{
						Complete = false;
					}
					else
					{
						if (a.ChangeCollor())
						{
							photonView.RPC("SetCollor",RpcTarget.All,a);
						}
					}
				}
			}
	...
	[PunRPC]
	void SetCollor(ST_PuzzleTile tile)
	{
		tile.SetCollor();
	}
